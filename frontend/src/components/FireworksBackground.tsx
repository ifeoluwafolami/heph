...
/** Draws a heart shape centered at (0, 0) after translation */
function drawHeartPath(ctx: CanvasRenderingContext2D, s: number) {
  ctx.beginPath();
  // Start at the top-center notch
  ctx.moveTo(0, -s * 0.5);
  // Left lobe
  ctx.bezierCurveTo(-s * 0.1, -s, -s, -s * 0.8, -s, -s * 0.3);
  // Left side down to bottom tip
  ctx.bezierCurveTo(-s, s * 0.1, 0, s * 0.5, 0, s);
  // Right side up from bottom tip
  ctx.bezierCurveTo(0, s * 0.5, s, s * 0.1, s, -s * 0.3);
  // Right lobe back to notch
  ctx.bezierCurveTo(s, -s * 0.8, s * 0.1, -s, 0, -s * 0.5);
  ctx.closePath();
}
...
function createParticle(
  ...
  return {
    ...
    draw(ctx: CanvasRenderingContext2D) {
      ...
      // Scale size up so hearts are visible (size * 2 gives a nicely visible heart)
      drawHeartPath(ctx, this.size * 2);
      ctx.fill();
      ctx.restore();
    },
    isAlive() {
